...
*Setup instructions for a gitian build of EncoCoin Core using a VM or physical system.*
...
to the EncoCoin GitHub release page.
...
- [Building EncoCoin Core](#building-encocoin-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/encocoin-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/encocoin-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/encocoin-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/encocoin-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/encocoin-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/encocoin-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/encocoin-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@kopernikus.dev) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/encocoin-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/encocoin-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/encocoin-osx-*-build.assert
```
